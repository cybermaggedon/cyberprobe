...
class eventstream_client {
  ...
  public:
    explicit eventstream_client(std::shared_ptr<Channel> channel)
        : stub_(cyberprobe::EventStream::NewStub(channel)), running(true) {}
    ...
    void observe(std::shared_ptr<cybermon::event::event> ev) {
        ...
        call->response_reader =
            stub_->PrepareAsyncObserve(&call->context, request, &cq_);
    ...
    void AsyncCompleteRpc() {
        ...
        std::cerr << "Async starting" << std::endl;
    ...
    void start_async() {
        thr = std::thread(&eventstream_client::AsyncCompleteRpc, this);
    }
  ...
  private:
    ...
    std::unique_ptr<cyberprobe::EventStream::Stub> stub_;
