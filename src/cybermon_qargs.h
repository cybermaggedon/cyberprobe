...
public:
	connection_args(const cybermon::context_ptr cp) : cptr(cp) {}
...
public:
	trigger_up_args(const std::string& liid, const std::string& a) : trupliid(liid), trupaddr(a) {}
	std::string trupliid;
...
public:
	trigger_down_args(const std::string& liid) : trdownliid(liid) {}
...
/*public:
	unrecognised_stream_args(const cybermon::context_ptr cp,
			cybermon::pdu_iter s,
			cybermon::pdu_iter e) : cptr(cp), pdus(s), pdue(e) {}
	cybermon::context_ptr cptr;
	cybermon::pdu_iter pdus;
	cybermon::pdu_iter pdue;*/
public:
	unrecognised_stream_args(const cybermon::context_ptr cp,
			cybermon::pdu data) : cptr(cp), pdu(data) {}
	cybermon::context_ptr cptr;
	cybermon::pdu pdu;
};
...
public:
	icmp_args(const cybermon::context_ptr cp,
			unsigned int type,
			unsigned int code,
			cybermon::pdu data) : cptr(cp), icmptype(type), icmpcode(code), icmpdata(data) {}
	cybermon::context_ptr cptr;
...
/*
	public:
		icmp_args(const cybermon::context_ptr cp,
				...
				unsigned int code,
				cybermon::pdu_iter s,
				cybermon::pdu_iter e) : cptr(cp), icmptype(type), icmpcode(code), pdus(s), pdue(e) {}
		cybermon::context_ptr cptr;
		unsigned int icmptype;
		unsigned int icmpcode;
		cybermon::pdu_iter pdus;
...
/*public:
	sip_request_args(const cybermon::context_ptr cp,
			const std::string& method,
			const std::string& from,
			const std::string& to,
			cybermon::pdu_iter s,
			cybermon::pdu_iter e) : cptr(cp), sipmethod(method), sipfrom(from), sipto(to), pdus(s), pdue(e)	{}
	cybermon::context_ptr cptr;
...
public:
	sip_request_args(const cybermon::context_ptr cp,
			...
			const std::string& from,
			const std::string& to,
			cybermon::pdu data) : cptr(cp), sipmethod(method), sipfrom(from), sipto(to), pdu(data)	{}
	cybermon::context_ptr cptr;
	const std::string sipmethod;
	const std::string sipfrom;
	const std::string sipto;
...
/*public:
	sip_response_args(const cybermon::context_ptr cp,
			unsigned int code,
			const std::string& status,
			const std::string& from,
			const std::string& to,
			cybermon::pdu_iter s,
			cybermon::pdu_iter e) : cptr(cp), sipcode(code), sipstatus(status), sipfrom(from), sipto(to), pdus(s), pdue(e){}
	cybermon::context_ptr cptr;
...
public:
	sip_response_args(const cybermon::context_ptr cp,
			...
			const std::string& from,
			const std::string& to,
			cybermon::pdu data) : cptr(cp), sipcode(code), sipstatus(status), sipfrom(from), sipto(to), pdu(data){}
	cybermon::context_ptr cptr;
	unsigned int sipcode;
	const std::string sipstatus;
	const std::string sipfrom;
...
/*public:
	http_request_args(const cybermon::context_ptr cp,
				const std::string& method,
				const std::string& url,
				const cybermon::observer::http_hdr_t& hdr,
				cybermon::pdu_iter body_start,
				cybermon::pdu_iter body_end) : cptr(cp), httpmethod(method), httpurl(url), httphdr(hdr), pdus(body_start), pdue(body_end) {}
		cybermon::context_ptr cptr;
...
public:
	http_request_args(const cybermon::context_ptr cp,
				...
				const std::string& url,
				const cybermon::observer::http_hdr_t& hdr,
				cybermon::pdu data) : cptr(cp), httpmethod(method), httpurl(url), httphdr(hdr), pdu(data) {}
		cybermon::context_ptr cptr;
		const std::string httpmethod;
		const std::string httpurl;
		cybermon::observer::http_hdr_t httphdr;
...
/*public:
	http_response_args(const cybermon::context_ptr cp,
			unsigned int code,
			const std::string& status,
			const cybermon::observer::http_hdr_t& hdr,
			const std::string& url,
			cybermon::pdu_iter body_start,
			cybermon::pdu_iter body_end) : cptr(cp), httpcode(code), httpstatus(status), httphdr(hdr), httpurl(url), pdus(body_start), pdue(body_end) {}
	cybermon::context_ptr cptr;
...
public:
	http_response_args(const cybermon::context_ptr cp,
			...
			const cybermon::observer::http_hdr_t& hdr,
			const std::string& url,
			cybermon::pdu data) : cptr(cp), httpcode(code), httpstatus(status), httphdr(hdr), httpurl(url), pdu(data) {}
	cybermon::context_ptr cptr;
	unsigned int httpcode;
	const std::string httpstatus;
	cybermon::observer::http_hdr_t httphdr;
...
public:
	smtp_command_args(const cybermon::context_ptr cp,
			const std::string& command): cptr(cp), smtpcommand (command){}
	cybermon::context_ptr cptr;
...
public:
	smtp_response_args(const cybermon::context_ptr cp,
			int status,
			const std::list<std::string>& text): cptr(cp), smtpstatus (status), smtptext(text){}
	cybermon::context_ptr cptr;
	int smtpstatus;
...
public:
	smtp_data_args(const cybermon::context_ptr cp,
			const std::string& from,
			const std::list<std::string>& to,
			std::vector<unsigned char>::const_iterator s,
			std::vector<unsigned char>::const_iterator e) :cptr(cp), smtpfrom(from), smtpto(to), smtps(s), smtpe(e) {}
	cybermon::context_ptr cptr;
...
public:
	ftp_command_args(const cybermon::context_ptr cp,
			const std::string& command) : cptr(cp), ftpcommand(command){}
	cybermon::context_ptr cptr;
...
public:
	ftp_response_args(const cybermon::context_ptr cp,
			int status,
			const std::list<std::string>& responses) : cptr(cp), ftpstatus(status), ftpresponses (responses) {}
	cybermon::context_ptr cptr;
	int ftpstatus;
...
public:
	dns_message_args(const cybermon::context_ptr cp,
			...
			const std::list<cybermon::dns_rr> additional) : cptr(cp), dnshdr(hdr), dnsqueries (queries), dnsanswers (answers), dnsauthorities (authorities), dnsadditional (additional){

		//std::cout<<"qargs cp:"<<cp.get()<<"\n";

		//std::cout<<"qargs dnshdr id:"<<dnshdr.id<<"\n";

		cybermon::dns_query dq = queries.front();
		//std::cout<<"qargs dns_query name:"<<dq.name<<"\n";

		//std::cout<<"qargs answers size:"<<answers.size()<<"\n";
		//std::cout<<"qargs authorities size:"<<authorities.size()<<"\n";
		//std::cout<<"qargs additional size:"<<additional.size()<<"\n";
		//std::cout<<"dns_rr name"<<rr.name;
...
public:
	ntp_timestamp_message_args(const cybermon::context_ptr cp,
			const cybermon::ntp_timestamp& ts) : cptr(cp), ntpts(ts){}
	cybermon::context_ptr cptr;
...
public:
	ntp_control_message_args(const cybermon::context_ptr cp,
			const cybermon::ntp_control& ctrl) : cptr(cp), ntpctrl(ctrl){}
	cybermon::context_ptr cptr;
...
public:
	ntp_private_message_args(const cybermon::context_ptr cp,
			const cybermon::ntp_private& priv) : cptr(cp), ntppriv(priv){}
	cybermon::context_ptr cptr;
...
enum call_type{
	...
	unrecognised_stream, unrecognised_datagram,
	icmp,
	imap,imap_ssl,
	pop3,pop3_ssl,
	rtp, rtp_ssl,

	sip_request, sip_response, sip_ssl,
	smtp_auth,smtp_command, smtp_response, smtp_data,
	http_request, http_response,
	ftp_command,ftp_response,
	dns_message,
	ntp_timestamp_message, ntp_control_message,ntp_private_message
...
public:
	q_entry(call_type call,qargs* args) : calltype(call),  queueargs(args){}
